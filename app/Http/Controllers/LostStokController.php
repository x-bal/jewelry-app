    ...
    public function lost(LostStok $lostStok)
    {
        if (request()->ajax()) {
            ...
            return DataTables::of($lostStok->barangs)
                ...
                ->addColumn('action', function ($row) {
                    $actionBtn = '<a href="#modal-detail" id="' . $row->id . '" class="btn btn-sm btn-info btn-show" data-bs-toggle="modal"><i class="ion-ios-eye"></i></a> <button type="button" data-route="' . route('detail-lost.destroy', $row->id) . '" class="delete btn btn-danger btn-delete btn-sm">Delete</button>';
                    return $actionBtn;
                })
